...
for patient in patients:
    mut = getMutations("data/AML_Mutations.txt", g, patient)
...
threshold = 1
targets = loadCausalGenes("data/AML_drug_targets.txt", g)
causal_gene_hits = 0
above_threshold_genes = 0
for gene in mut_dict:
    if mut_dict[gene] > threshold:
        above_threshold_genes += 1
        if gene in targets:
            causal_gene_hits += 1
            print(gene)
print('hit ' + str(causal_gene_hits) + ' out of ' + str(len(targets)) + '(above threshold: ' + str(above_threshold_genes) + ')')
print('score: ' + str(float(causal_gene_hits) / len(targets)))
p = stats.hypergeom.sf(causal_gene_hits, gene_num, len(targets), above_threshold_genes) + \
    stats.hypergeom.pmf(causal_gene_hits, gene_num, len(targets), above_threshold_genes)
print('p-value: ' + str(p))
